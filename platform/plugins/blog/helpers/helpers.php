...
if (!function_exists('get_partners')) {
     ...
     * @return array
     */
    function get_partners($limit)
    {
        return app(PartnersInterface::class)->getPartners($limit);
    }
}
...
if (!function_exists('get_circuits_with_children')) {
    /**
     * @return array
     * @throws Exception
     */
    function get_circuits_with_children()
    {
        $circuits = app(CircuitInterface::class)
            ->getAllCircuitsWithChildren(['status' => BaseStatusEnum::PUBLISHED], [], ['id', 'name', 'parent_id']);
        $sortHelper = app(SortItemsWithChildrenHelper::class);
        $sortHelper
            ->setChildrenProperty('child_cats')
            ->setItems($circuits);
